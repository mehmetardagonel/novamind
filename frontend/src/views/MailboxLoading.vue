...
export default {
  ...
  setup() {
    ...
    const { bootStatus, bootPercent, bootErrorMessage } = storeToRefs(emailsStore);
    ...
    const ensureAccountsAndPrefetch = async ({ force = false } = {}) => {
      ...
      const MIN_LOADING_TIME = 1500; // Minimum 1.5 seconds for smooth progress bar
      ...
      await emailsStore.prefetchMailboxAfterLogin({ force });
      if (emailsStore.bootStatus === "ready") {
        // Calculate remaining time to reach minimum loading duration
        const elapsedTime = Date.now() - startTime;
        const remainingTime = Math.max(0, MIN_LOADING_TIME - elapsedTime);

        // Wait for remaining time + extra for cache write
